...
В начале каждого модуля находится самое главное: импорт, экспорт, интерфейсы и конструкторы.
...
<p style="font-size: 70px; text-align:left; padding:100px;">
    <i>Модуль архитектурного слоя не должен ничего импортировать из модулей внешних слоёв. <br/> При этом он может импортировать
       из модулей своего слоя и из модулей внутренних по отношению к нему слоёв.</i>
</p>


<Notes>

*Правило Зависимостей*, переведённое на язык с поддержкой модульности, каким, конечно, является и *TypeScript*, звучит
так:

*Модуль архитектурного слоя не должен ничего импортировать из модулей внешних слоёв. При этом он может импортировать
из модулей своего слоя и из модулей внутренних по отношению к нему слоёв.*
...
Начнём с бизнес-логики. Рассмотрим модуль *CounterIncrement*.

Обратите внимание на первую строку - она содержит импорт класса *Counter* из слоя сущностей.
С точки зрения 
