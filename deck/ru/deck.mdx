...
Сразу стоит сказать, что презентация развёрнута на надёжном хостинге (в конце будет дана ссылка и на её исходник), имеет
высокую степень самодостаточности, поскольку сопровождается техническим текстом, поэтому для её восприятия наличие
докладчика не требуется и записывать её на аудио/видео не обязательно.
...
```ts 13:30 file=../../src/domain/usecases/CounterIncrement.ts title="CounterIncrement"
```
...
При этом важно отметить, что большая часть решений будет применима и в других языках, поддерживающих статическую
типизацию, таких как *Java*, *Kotlin*, *C#* и т.п.   
...
На слайде представлены модули - представители разных архитектурных слоёв.
Они показаны вместе, чтобы легче было увидеть их взаимосвязь по зависимостям.

В начале каждого модуля с архитектурной точки зрения находится самое главное: интерфейсы, импорт, экспорт и конструкторы
(так и должно быть, хотя *TypeScript* этого не требует). Ниже - реализация, но сейчас речь не о ней.  

На слайде у модулей выделены только строки, имеющие отношение к соблюдению *Правила Зависимостей*. 

В центральном столбце размещён *CounterIncrement* - модуль бизнес-логики из слоя *Use Cases*.

По краям - модули из слоя адаптеров: слева - *useIncrementPresenter* - презентер для *React*, реализованный как
*custom hook*. Справа - *CounterIncrementRestGateway* - шлюз доступа к внешнему сервису, сохраняющему и возвращающему
