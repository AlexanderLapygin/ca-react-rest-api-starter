...
На это несколько причин, не самая последняя из которых - популярность *React* среди разработчиков.
...
Стартер написан на *TypeScript*, на выбор которого повлияли примерно те же причины, что и на выбор *React*.

В среде frontend-разработчиков *TypeScript* уже давно имеет многочисленных сторонников, а последнее время начинает
