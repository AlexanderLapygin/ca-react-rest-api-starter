...
Например, можно сделать её плоской, упразднив корневые директории и разместив оставшиеся на одном уровне:  
...
Это клиентская часть приложения, обведём её пунктиром.
...
Для полноты картины разместим на диаграмме и серверную часть, реализующую REST API для сохранения значения счётчика,
