...
Это первая и, хочется надеяться, не последняя из серии презентаций о применении принципов *Чистой Архитектуре* во
фронтенд-разработке.

Конечно, было бы неплохо сначала рассказать о том, как *Чистая Архитектура* пришла во фронтенд, какое место она занимает
там сегодня, и каковы дальнейшие перспективы. Но, как говорится, пока есть к чему стремиться.
 
Разговор пойдёт вокруг клиентского приложения - стартера, предметная область и, соответственно, бизнес-логика которого
предельно минималистичны - это обычный счётчик с функциями инкремента и сброса.
Но при всей своей минималистичности стартер представлен fullstack-решением, в котором клиентская часть взаимодействует
с persistence-сервисом (назовём это так) по REST API и имеет несколько аспектов, требующих отдельного рассмотрения.
 
В этой презентации главное внимание уделяется *Правилу Зависимостей* *Чистой Архитектуры*, в частности, тому, как оно
может быть представлено в программном коде, в данном случае - на *TypeScript*. 
...
Несмотря на то, что пользовательский интерфейс стартера реализован на *React*, для понимания того, о чём здесь будет
говориться, знания *React* не потребуются.   
...
Вот, например, есть такой спорт для программистов, называется *хакатон* (сокращённо - *хакерский марафон*).

Его суть состоит в том, что в условиях ограниченного времени, обычно в пределах одной-двух недель, разрабатывается
программа, решающая какую-то проблему.

Вот только у хакатона неправильное название. Для написания хоть сколько-нибудь серьёзной программы, две недели это не
марафон, конечно, а в чистом виде спринт.
...
Так вот сравнивать скорость разработки на хакатоне и скорость разработки на принципах *Чистой Архитектуры* это всё равно
что сравнивать скорость спринтера и стайера, велосипеда и поезда.

При старте с места велосипед без вопросов выиграет у поезда на дистанции 100 метров. Выиграет и на 200 метров, и на 1000.
...
хотя оно не является изобретением *Чистой Архитектуры*. По сути оно присутствовало и в [*Гексагональной
...
Код написан на *TypeScript*, что также обусловлено популярностью этого языка.
...
Далее речь пойдёт уже о т*Правиле Зависимостей*, поэтому
...
Конечно, на функциональности счётчика для web-платформы можно показать, каким должен быть самый старт разработки, чтобы
с самого начала не лишиться перспектив на развитие, но этого слишком мало для того, чтобы преимущества *Чистой
Архитектуры* успели проявиться.

Напомним, что разработка на принципах *Чистая Архитектура* используется при игре в долгую. Преимущества *Чистой
Архитектуры* по-настоящему проявляются не на старте разработки, а в ходе развития системы.

Поэтому в продолжение этой презентации запланировано ещё несколько, на первой из которых предполагается демонстрация
развития показанного здесь решения для мобильных платформ, конкретно - на *React Native*. 
