...
В этой презентации и в коде самого стартера используются именования, вполне традиционные для Чистой Архитектуры, но для
тех, кто только начинает знакомиться с этой областью, некоторые понятия нуждаются в пояснениях.

Не погружаясь глубоко в детали, приведём краткий словарь используемых понятий с соответствующими комментариями.
...
Ответственности* (*SRP*), который, в свою очередь, является одним из самых главных принципов *SOLID* и, соответственно,
Чистой Архитектуры.
...
*Чистой Архитектуры* адаптеры часто служат для согласования интерфейсов внутреннего и внешнего архитектурных слоёв.
При этом наиболее типовым случаем является согласование интерфейсов фреймворков и внешних систем с интерфейсами
*бизнес-логики*.
В коде предлагаемого стартера используются *адаптеры* двух видов:  *презентеры* и *шлюзы*(*presenters* и *gateways*).

- *Presenter* - по сути - адаптер пользовательского интерфейса для модулей бизнес-логики.

- *Gateway* - адаптер некоторого сервиса, используемого бизнес-логикой, в наиболее типовых случаях доступного через REST
API. В частности, сервис может выполнять функции хранения состояния сущностей (persistence), иногда в таких случаях
вместо *Gateway* используется термин *Repository*.
