...
Архитектурой* и очень желательно - знакомство с материалом, изложенным в презентации
[*О Принципе Инверсии Зависимостей*](https://csb-9kg5y-8d0n41fyp.now.sh).
...
В начале модуля размещены определения *provided-* и *consumed*-интерфейсов модуля, названных, соответственно,
*CounterResetIn* и *CounterResetOut*.

Интерфейс *CounterResetOut* является *требуемым* (*required*) интерфейсом для адаптера из соседнего внешнего слоя.
