...
Состояние счётчика сохраняется с помощью внешнего сервиса, взаимодействие с которым осуществляется по REST API.  
...
Иногда совокупность *Use Case* понимается как слой *бизнес-логики*, относящийся к конкретному программному продукту - в
противовес *Entities* - слою бизнес-логики, разделяемому множеством продуктов.
...
- *Infrastructure* - инфраструктура - всё, что находится во внешнем слое.

К типичным представителям *Инфраструктуры* относятся базы данных (*DB*) и пользовательские интерфейсы (*UI*) со своими
фреймворками.
