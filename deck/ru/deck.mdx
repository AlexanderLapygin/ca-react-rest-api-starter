...
в данном случае, в коде *TypeScript* - языка, который в среде frontend-разработчиков сегодня имеет многочисленных
сторонников, и который последнее время начинает завоёвывать позиции уже и на backend. 
...
В качестве UI-фреймворка используется *React*. На это несколько причин, не самая последняя из которых - популярность
*React* среди разработчиков.
