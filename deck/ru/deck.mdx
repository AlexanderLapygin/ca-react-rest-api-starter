...
Пользовательский интерфейс стартера реализован на *React*, но понимания того, о чём здесь будет говориться, знания
*React* не требуется.   
...
*Чистая Архитектура* это очень просто. Из всех её принципов наиболее значимых всего два.

Главный из этих принципов здесь будет рассмотрен, после чего тревожные ожидания покинут тех, у кого они были, и многим
наверняка захочется сразу же применить полученные знания на практике. 
...
Архитектуры*, они будут непривычны для тех, кто только начинает знакомиться с этой областью, поэтому они нуждаются в
...
Возможно, кто-то скажет, что *Правило Зависимостей*, мол, очевидно, банально, что он и так всегда его соблюдал - такие
всегда находятся, кстати, именно среди хороших программистов.

Но практика показывает, что на самом деле это не так, таких опытных разработчиков с хорошей интуицией - единицы, а
постоянное нарушение этого правила, низкая культура работы с зависимостями пока ещё является нашей повседневной
реальностью.
...
*Чистая Архитектура* никак не регламентирует ни названия, ни число слоёв. На оригинальной диаграмме показан просто самый
типовой пример, который и нужно воспринимать как пример, не более того.

Но в любой системе всегда есть две составляющие: модель предметной области - *Domain* и  инфраструктура -
*Infrastructure*. 

Задача "чистого" архитектора заключается в том, чтобы *Domain* и *Infrastructure* декомпозировать на слои,
соответствующие решаемой задаче. 

*Правило зависимостей* заставляет разработчика с самого начала думать о важных вещах: о том, где и как провести границу
между сущностями предметной области и сценариями бизнес-логики, между бизнес-логикой продукта и инфраструктурой, которая,
как раз, и состоит из библиотек, фреймворков и баз данных.
...
<p style="font-size: 70px; text-align:left; padding:100px;">
    ...
    <p style="font-size: 50px; text-align:right; color:#addb67;"><i>Правило Зависимостей в терминах языка программирования</i></p>
</p>
